...
@app.route("/gerar-grade", methods=["POST"])
def gerar_grade():
    ...
    for turma in dados["turmas"]:
        ...
        horarios_str = (
            "["
            + ",".join([f"({dia},{horario})" for dia, horario in horarios_turma])
            + "]"
        )
    ...
    try:
        ...
        saida = json.loads(resultado.stdout.decode())
        erro = resultado.stderr.decode()
        if erro:
            return jsonify({"erro": erro}), 200
        if "erro" in saida:
            return jsonify({"erro": saida["erro"]}), 200
        else:
